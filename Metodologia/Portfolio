...
O projeto Falcon teve como objetivo introduzir os alunos ao desenvolvimento de software, versionamento e organização de projetos. A solução desenvolvida consistiu em um sistema simples, mas fundamental para praticar lógica de programação, estruturação de diretórios, colaboração e uso correto do GitHub.
...
O projeto Porygon apresentou uma evolução significativa em relação ao primeiro semestre, introduzindo o uso de Java com Spring Boot e MySQL, aprofundando o conhecimento sobre APIs REST, MVC e modelagem de dados. O sistema consistia no cadastro e gerenciamento de funcionários e setores, com páginas renderizadas via Thymeleaf.
...
(Este é exatamente o texto que você enviou, sem nenhuma modificação.)
...
O projeto Porygon3 deu continuidade ao ecossistema desenvolvido nos semestres anteriores, expandindo funcionalidades, melhorando desempenho e incorporando novos módulos, incluindo mapas interativos e dashboards. O sistema passou a contar com arquitetura mais robusta, integração com componentes reativos e melhorias significativas na experiência do usuário.
...
Axios – Comunicações HTTP entre frontend e backend.
...
Trabalhei na integração completa entre frontend e backend usando Axios, garantindo consistência entre DTOs e objetos retornados.
